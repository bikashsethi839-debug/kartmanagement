...
class CustomHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def serve_static_file(self, path):
        ...
        file_path = 'frontend' + path  # path already has leading slash
    ...
    def handle_api_request(self, path):
        # Initialize services here to avoid issues with request handler initialization
...
def main():
    ...
    # Default to HTTPS using local certs in `certs/` (dev key + crt). Use --no-https to disable.
    cert_dir = os.path.join(os.getcwd(), 'certs')
    cert_key = os.path.join(cert_dir, 'dev.key')
    cert_crt = os.path.join(cert_dir, 'dev.crt')

    if not args.no_https:
        # Ensure certs exist; try to generate a self-signed cert if missing.
